...
local function generate_share_url()
    ...
    if platform == "github" or platform == "gitlab" then
        share_url = string.format(
            "%s/blob/%s/%s#L%d-L%d",
            base_url, current_branch, relative_path, start_line, end_line
        )
...
vim.keymap.set('v', '<leader>gu', generate_share_url,
    { noremap = true, silent = true, desc = "Generate share URL" })
