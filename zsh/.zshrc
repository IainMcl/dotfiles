# OPENSPEC:START
# OpenSpec shell completions configuration - fpath must be set before oh-my-zsh calls compinit
fpath=("/Users/iain.mclaughlan/.oh-my-zsh/custom/completions" $fpath)
# OPENSPEC:END
...
zstyle ':vcs_info:*' enable git    # skip hg/bzr/svn detection on every prompt
ZSH_DISABLE_COMPFIX=true           # skip compaudit security check (~234ms)
...
# Lazy-load NVM - defers ~500ms startup cost until first use
...
# Lazy-load thefuck - defers ~300ms Python startup cost until first use
fuck() {
    unfunction fuck
...
_pyenv_cache="${HOME}/.cache/pyenv_init.zsh"
if [[ ! -f "$_pyenv_cache" || -z "$(find "$_pyenv_cache" -mmin -10080 2>/dev/null)" ]]; then
    mkdir -p "${HOME}/.cache"
    pyenv init - > "$_pyenv_cache"
    pyenv virtualenv-init - >> "$_pyenv_cache"
fi
source "$_pyenv_cache"
unset _pyenv_cache
...
if [[ "$(hostname)" == "$WORK_HOST" ]]; then
    ...
    # NVM via homebrew (overrides the lazy loader above on this host)
    nvm() {
        unfunction nvm node npm npx 2>/dev/null
    ...
    _tk_cache="${HOME}/.cache/tk_shell_setup.zsh"
    if [[ ! -f "$_tk_cache" || -z "$(find "$_tk_cache" -mmin -480 2>/dev/null)" ]]; then
        mkdir -p "${HOME}/.cache"
        tk shell-setup > "$_tk_cache"
    fi
    source "$_tk_cache"
    unset _tk_cache
    ...
    test_backend () {
        ...
        then
            ...
            echo "Example:"
            echo "    test_backend payments/services/tests/test_vcc_provider_benefit_service.py"
            echo "    test_backend DEBUG payments/services/tests/test_vcc_provider_benefit_service.py"
            echo "    test_backend DEBUG payments/services/tests/test_vcc_provider_benefit_service.py::VCCProviderPreferenceTestCase"
            return
        fi
